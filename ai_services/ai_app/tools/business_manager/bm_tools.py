...
#                 "reorder_amount": item.threshold * 2,
#             }
#             db.add(
#                 Approval(
#                     type="Purchase Order",
...
#             [
#                 f"- {s.name} (Rating: {s.rating}, Lead: {s.lead_time_days}d)"
#                 for s in suppliers
#             ]
#         )
#     except Exception as e:
...
#         }
#         httpx.post(
#             "http://127.0.0.1:5678/webhook/invite-user", json=payload
#         ).raise_for_status()
#         return f"Successfully dispatched an invitation email to {name} ({email}) for the role of {role}."
#     except Exception as e:
#         return f"Failed to dispatch invitation: {str(e)}"
...
#         }
#         httpx.post(
#             "http://127.0.0.1:5678/webhook/low-stock-alert", json=payload
#         ).raise_for_status()
