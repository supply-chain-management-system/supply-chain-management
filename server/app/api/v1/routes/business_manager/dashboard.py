...
def get_requests(
    db: Session = Depends(get_db), user=Depends(require_role(["Business Manager"]))
):
    """
    ...
    try:
        ...
        pending_requests = (
            db.query(Approval)
            .filter(Approval.status == "PENDING_WHM_APPROVAL")
...
async def create_restock_request_from_n8n(
    data: N8nRestockPayload, db: Session = Depends(get_db)
):
    """
    ...
    try:
        ...
        return {
            "status": "success",
            "message": f"In-app restock request created for {data.product_name}.",
        }
...
def process_request_action(
    ...
    return {
        "status": "success",
        "message": f"Request {request_id} has been {req.status}.",
    }
