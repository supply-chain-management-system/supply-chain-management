...
from app.models.factory_manager.production import Production, Production_status, Factory
...
def create_factory_production_draft(product_name: str, target_qty: int, factory_id: int, user_id: int, company_id: int) -> str:
    ...
    try:
        ...
        if not factory:
            return f"Access Denied: Factory {factory_id} does not belong to your company."
...
def bulk_approve_requests(filter_type: str, filter_value: str, user_id: int) -> str:
    ...
    try:
        ...
        db.commit()
        return f"Successfully processed bulk approval. {count} requests are now APPROVED."
    except Exception as e:
        db.rollback()
...
def check_supplier_status(max_rating: float, min_lead_time_days: int) -> str:
    ...
    try:
        ...
        for s in suppliers:
            result_str += f"- {s.name} (Rating: {s.rating}, Lead Time: {s.lead_time_days} days)\n"
...
def invite_team_member(name: str, email: str, role: str, business_name: str) -> str:
    ...
    invite_link = f"http://localhost:5173/setup-account?token={mock_token}&email={email}"
